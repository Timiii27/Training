...
export function workoutTypeLabel(key) {
  return workoutTypeOptions.find((option) => option.key === key)?.label || "Entreno";
}
...
// Detecta ejercicios por tiempo ("40 s", "30-45 s / lado") y devuelve los
// segundos objetivo (tope del rango). Devuelve null para ejercicios por reps.
export function timedSecondsFor(reps) {
  const match = /(?:(\d+)\s*-\s*)?(\d+)\s*s(?![a-z])/i.exec(String(reps || ""));
  if (!match) return null;
  return Number(match[2]);
}

// Segundos restantes de la serie en marcha (ejercicios por tiempo).
export function workLeftFor(workout, now = Date.now()) {
  if (!workout?.workEndsAt) return 0;
  return Math.max(0, Math.ceil((workout.workEndsAt - now) / 1000));
}

// Deriva el segundo fotograma del primero cuando la fuente sigue el patron
// .../0.jpg <-> .../1.jpg (free-exercise-db), para animar el movimiento sin
// necesidad de una segunda columna en la base de datos.
function secondFrame(url) {
  if (!url) return null;
  const next = url.replace(/\/0(\.(?:jpe?g|png|webp))(\?.*)?$/i, "/1$1$2");
  return next !== url ? next : null;
}

export function deriveFrames(imageUrl, imageUrl2) {
  const explicit = [imageUrl, imageUrl2].filter(Boolean);
  if (explicit.length > 1) return explicit;
  if (!imageUrl) return ["/assets/exercises/pushup.jpg"];
  return [imageUrl, secondFrame(imageUrl)].filter(Boolean);
}
...
export function normalizeRoutine(row) {
  ...
  const exercises = [...(row.routine_exercises || [])]
    ...
    .map((exercise) => ({
      ...
      frames: deriveFrames(exercise.image_url, exercise.image_url_2),
